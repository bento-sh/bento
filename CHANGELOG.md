...
- **Cache keys are sound by default — every existing key invalidates once (`FORMAT_TAG` v2).** Adapter default `inputs` for cargo / node-family / maven / gradle are now `**` (whole dish tree) minus derived dirs, instead of narrow allowlists (`src/**`, `public/**`, …) that left a Next.js `app/`, a cargo workspace's `crates/*/src`, a Maven `<modules>` root or a Gradle multi-project root unhashed. The input walk prunes `.git` / `.bento` / `node_modules` / `target` / `dist` / `build` / `.next`, each adapter's `derived_paths()` (new lists for node-family, cargo, gradle, maven, php `vendor/**`), and the task's own `outputs`; symlinks hash by target path. Dish-level `inputs` is unioned into adapter lifecycle tasks (was silently shadowed). Keys now also mix in `outputs` / `workspace_outputs`, the dish's workspace-relative path, the *resolved* container plan (runtime + image, or native — the same resolution the executor uses, so a laptop without docker and CI with docker never share a key), and the aliased *source* env value under `bento deploy --env` / `--secret-from` (staging and prod builds previously shared a key). `outputs = ["dist/"]` normalises to `dist/**` (a trailing-slash literal matched nothing, so hits restored nothing). A custom task on a no-adapter dish with no `inputs` is uncacheable instead of hitting forever on a constant key. Ruby's `Gemfile.lock` is hashed again (it was listed as derived, so `bundle update` never moved the key).
- **`bento ci` runs lifecycle tasks only.** `bento init` mirrors every `package.json` script into `[tasks.<name>]`, and `bento ci` ran all of them — a fresh init on a Next.js app made `bento ci` spawn `npm run dev` and hang. Custom-named tasks are now `bento run`-only unless they set the new `ci = true`; new per-task `cache = false` (Turborepo's `cache: false`) always runs and never restores.
- **`--json` is compact when stdout is a pipe**, pretty when it's a terminal — ~45% fewer bytes (= tokens) for agents. Failed tasks no longer repeat stderr in `output_excerpt`.
- **`--since` is a `bento plan` flag**, no longer global — only plan ever read it; `bento ci --since` silently built everything.
- `bento --help`: one-line summaries per verb; the top-level about named five ecosystems out of thirteen. Human-mode errors now print the same `hint` + `next_steps` the JSON envelope carries.

### Fixed

- **Cache bundles can no longer write outside the dish.** `unpack_at` accepted symlink / hard-link entries and only screened the entry path for `..`; a hostile bundle from any remote tier could redirect a later entry outside the anchor root. Non-regular entries are refused and the resolved parent must stay under the root. `bento cache clear` only removes bento's own artefacts (with `BENTO_CACHE_DIR` pointed at a shared dir it was `rm *`). A torn or corrupt bundle is discarded and rebuilt instead of failing the task on every run until `cache clear`.
- **JS workspace members are detected and don't reinstall every run.** Node-family detection required the lockfile in the dish dir, which workspace members never have — `bento init` on any npm / pnpm / yarn / bun monorepo found zero JS dishes. Detection now needs `package.json` and disambiguates by the corepack `packageManager` field, then the nearest ancestor lockfile, then npm. The install probe runs against `install_scope` (the workspace root), and packages with nothing to install probe Ready.
- **Structured diagnostics fire for cargo, eslint and golangci-lint.** cargo's `--message-format=json` landed after `-- -D warnings` (on clippy-driver); eslint was invoked bare (never on PATH); golangci-lint v2 removed `--out-format`. Only ruff worked before.
- **Toolchains resolve on the executor thread before the dish fan-out** — `ensure_toolchain` mutated PATH via `set_var` from worker threads (a getenv/setenv data race), and two dishes pinning the same tool raced on the download.
- `bento-mcp`: the `instructions` string served on every connect still said "Today only the `ping` scaffold tool is wired"; read-only tools carried no annotations (MCP defaults = destructive + open-world). All 16 tools now declare hints; `ping` is gone; docs name the tools correctly (`prime`, not `bento_prime`).
- `bento dish add ../x` / `/abs/x` wrote outside the workspace and registered an unloadable ref.
- Tasks skipped by `fail_fast` appear as `skipped` rows in the report instead of vanishing.
